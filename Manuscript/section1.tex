...
With the rapid development of the wireless devices, it is possible to estimate the occupancy of a certain area by observing the Wi-Fi packets. 
This article focuses on occupancy estimation based on Wi-Fi environment. Several sensing devices(network cards) are switched into monitoring mode to collect local package from Wi-Fi enabled devices.
Under that mode, the sensing devices are able to get all data packets they receives. The sensing clients will abstract MAC addresses as well as RSSIs from data packets.
The estimator takes Mac addressed an RSSIs as input and estimates the number of active devices in the area. This work emphasised on finding the certain positions which will have the best result to place the sensing devices.
